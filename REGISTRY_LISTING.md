...
- [ ] Confirm `package.json`: `name` `@edgedepth/research-mcp`, `version` matches the release, `mcpName` `com.edgedepth/research`, `bin`, and `files` (`dist`, `README.md`, `server.json`).
...
- [ ] Verify: `https://www.npmjs.com/package/@edgedepth/research-mcp` shows the released version, and `npx -y @edgedepth/research-mcp` starts (set `EDGEDEPTH_API_KEY` first).

Version bumps: publishing a new version means synchronize `package.json`, both root package entries in `package-lock.json`, `src/version.ts`, and the top-level and package versions in `server.json`, then republish npm, then re-run the registry publish (step 3).
...
- [ ] Cursor: paste the `~/.cursor/mcp.json` remote block from the README; confirm `/mcp` shows the tool inventory pinned by `test/tools.schema.test.ts` and `list_features` returns the registry.
